...
ETAPA 1 DO DECAP CMS:
- O layout do Blog fica aqui.
- Os posts agora vêm de arquivos Markdown em:

  src/content/posts/

- O arquivo que lê os posts fica em:

  src/content/blogPosts.js

Quando o Decap CMS for instalado, ele vai criar e editar esses
arquivos Markdown pelo painel /admin.
============================================================
*/
...
function isAvailable(post) {
  return post.link && post.link !== '#' && !post.draft;
}
...
export default function BlogPage({ onStartQuiz }) {
  ...
  const filteredPosts = useMemo(() => {
    const postsWithoutFeatured = blogPosts.filter((post) => post.slug !== featuredPost?.slug);
